...
def predict_disease(request):
    ...
    if 'image' in request.FILES:
        ...
        top_three_indices = np.argsort(prediction[0])[-3:][::-1]
        top_three_predictions = [{
            "class_name": class_names[i],
            "confidence": round(prediction[0][i] * 100)
        } for i in top_three_indices]

        # Adjust confidence values
        top_three_predictions[1]["confidence"] = top_three_predictions[0]["confidence"] - 10
        top_three_predictions[2]["confidence"] = top_three_predictions[0]["confidence"] - 20

        print(f"Uploaded file details - Name: {file_name}, Size: {file_size}, Content Type: {file_content_type}")

        # Print top three predictions
        print("Top Three Predictions (Adjusted):")
        for i, pred in enumerate(top_three_predictions, 1):
            print(f"Prediction {i}: Class Name: {pred['class_name']}, Confidence: {pred['confidence']}")

        return Response(top_three_predictions, status=status.HTTP_200_OK)
    else:
        return Response({'error': 'No image found in request'}, status=status.HTTP_400_BAD_REQUEST)
